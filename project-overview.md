...
```
AI 客服项目_md/
├── AI 客服项目总体说明.md          # 技术路线对比
├── 整理的需求文档.md                # 业务需求
├── 功能模块 + NFR + 优先级.md       # 功能清单
├── 第三方客服系统技术方案与迁移计划.md  # Chatwoot 路线方案
└── 项目报告/
    ├── 综合分析报告.md              # 技术分析
    └── 项目周报/                    # 周报记录

projects/
├── asair-admin-backend/            # 管理后台后端
├── asair-admin-frontend/            # 管理后台前端
├── asair-ai-server/                 # AI 大脑服务
└── asair-infra/                     # 基础设施
```
...
```bash
# 后端服务
cd asair-admin-backend && python -m asair_admin_backend.app
cd asair-ai-server && python -m asair_ai_server.app

# 前端服务
cd asair-admin-frontend && npm run dev
```

**健康检查：**
```bash
curl http://localhost:8100/healthz  # admin-backend
curl http://localhost:8000/healthz  # ai-server
```

---

**报告生成时间：** 2025-01-XX  
**下次更新建议：** 每周更新一次，记录关键进展和问题
